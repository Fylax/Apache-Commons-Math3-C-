...
namespace Math3.linear
{
    ...
    /// <para>Decomposition algorithms decompose an A matrix has a product of several specific
    /// matrices from which they can solve A &times; X = B in least squares sense: they find X
    /// such that ||A &times; X - B|| is minimal.</para>
    /// <para>Some solvers like <see cref="FieldLUDecomposition"/> can only find the solution for
    /// square matrices and when the solution is an exact linear solution, i.e. when
    /// ||A &times; X - B|| is exactly 0. Other solvers can also find solutions
    /// with non-square matrix A and with non-null minimal norm. If an exact linear
    /// solution exists it is also the minimal norm solution.</para>
    /// </summary>
    /// <typeparam name="T">the type of the field elements</typeparam>
    public interface FieldDecompositionSolver<T> where T : FieldElement<T>
    {

        /// <summary>
        /// Solve the linear equation A &times; X = B for matrices A.
        /// <para>The A matrix is implicit, it is provided by the underlying
